...
/**
 * Panel reutilizable para todos los métodos de integración numérica. Cambia de
 * comportamiento según el metodoId recibido.
 */
public class MetodoPanel extends JPanel {
    ...
    private JPanel crearBotonesRapidos() {
        ...
        // [etiqueta, textoAInsertar, retrocederCursorN posiciones]
        // Para raíces n-ésimas se usa pow(base, 1.0/n) porque exp4j no tiene cbrt ni nthroot
        String[][] btns = {
            {"x²", "x^2", "0"},
            {"x³", "x^3", "0"},
            {"√", "sqrt()", "1"}, // cursor queda dentro: sqrt(|)
            {"³√", "cbrt()", "1"}, // inserta cbrt(|)
            {"⁴√", "nthrt(,4)", "2"}, // inserta nthrt(|,4)
            {"sin", "sin()", "1"},
            {"cos", "cos()", "1"},
            {"tan", "tan()", "1"},
            {"log", "log()", "1"},
        ...
        for (String[] b : btns) {
            ...
            // Tooltip descriptivo según el botón
            String tooltip = switch (b[0]) {
                case "³√" ->
                    "Raíz cúbica: pow(x, 1.0/3)";
                case "⁴√" ->
                    "Raíz cuarta: pow(x, 1.0/4)";
                case "√" ->
            ...
            final int retroceder = Integer.parseInt(b[2]);
            ...
            btn.addActionListener(e -> {
                ...
                // Posicionar el cursor DENTRO del paréntesis
                txtFuncion.setCaretPosition(pos + insertar.length() - retroceder);
    ...
    private void calcular() {
        ...
        SwingWorker<ResultadoIntegracion, Void> worker = new SwingWorker<>() {
            @Override
            protected ResultadoIntegracion doInBackground() {
    ...
    private ResultadoIntegracion ejecutarMetodo(String funcion, double a, double b, int n) {
        return switch (metodoId) {
            case MainFrame.PANEL_TRAPECIO ->
                MetodosIntegracion.trapecio(funcion, a, b, n);
            case MainFrame.PANEL_BOOLE ->
                MetodosIntegracion.jorgeBoole(funcion, a, b);
            case MainFrame.PANEL_SIMPSON13 ->
                MetodosIntegracion.simpson13(funcion, a, b);
            case MainFrame.PANEL_SIMPSON38 ->
                MetodosIntegracion.simpson38(funcion, a, b);
            case MainFrame.PANEL_SIMP_ABIERTO ->
                MetodosIntegracion.simpsonAbierto(funcion, a, b, n);
            default ->
                throw new IllegalStateException("Método desconocido: " + metodoId);
        };
    }
    ...
    private String obtenerNombreMetodo() {
        return switch (metodoId) {
            ...
            case MainFrame.PANEL_BOOLE ->
                "Método Jorge Boole";
            case MainFrame.PANEL_SIMPSON13 ->
                "Método Simpson 1/3 - Newton-Cotes";
            case MainFrame.PANEL_SIMPSON38 ->
                "Método Simpson 3/8 - Newton-Cotes";
            case MainFrame.PANEL_SIMP_ABIERTO ->
                "Simpson Abierto (n par)";
            default ->
    ...
    private String obtenerFormula() {
        return switch (metodoId) {
            case MainFrame.PANEL_TRAPECIO ->
                "I = (Δ/2)[f(x₀) + 2f(x₁) + ... + f(xₙ)]";
            case MainFrame.PANEL_BOOLE ->
                "I = (2Δ/45)[7f(x₁)+32f(x₂)+12f(x₃)+32f(x₄)+7f(x₅)]";
            case MainFrame.PANEL_SIMPSON13 ->
                "I = (Δ/3)[f(x₁) + 4f(x₂) + f(x₃)]";
            case MainFrame.PANEL_SIMPSON38 ->
                "I = (3Δ/8)[f(x₁)+3f(x₂)+3f(x₃)+f(x₄)]";
            case MainFrame.PANEL_SIMP_ABIERTO ->
                "I = (Δ/3)[f(x₁)+4f(x₂)+2f(x₃)+4f(x₄)+...+f(xₙ)]";
            default ->
                "";
        };
    }
